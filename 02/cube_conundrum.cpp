...
export struct Cubes {
    ...
    friend bool operator==(Cubes const&, Cubes const&) = default;
};
...
export std::ostream& operator<<(std::ostream& os, Cubes const& c) {
...
export std::vector<Game> parseInput(std::string_view input)
{
    std::vector<Game> ret;
...
export bool isPossible(Cubes const& guess, Game const& game) {
    return std::ranges::all_of(game.reveals, [&guess](Cubes const& c) { return isPossible(guess, c); });
}

export int answer1(std::vector<Game> const& games)
{
...
export int64_t power(Game const& g)
{
    Cubes max{};
...
export int64_t answer2(std::vector<Game> const& games)
{
